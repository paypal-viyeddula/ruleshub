 ...
 * 
 * IMPORTANT: Fastlane in v6 SDK uses createInstance() with CLIENT TOKEN
 * This is different from v4 which uses script tag and Client ID in frontend
 * 
 * UPDATED: November 2024 - After complete debugging and successful implementation
 */

const FASTLANE_GUIDE_v4_to_v6 = `
...
### Error 5: Authentication state "failed"

**Symptoms:** Seeing "Authentication failed - showing guest experience" in console

**Causes:**
1. User didn't complete OTP verification
2. User entered wrong OTP
3. OTP expired

**Solution:** This is expected behavior. Always provide fallback to guest experience:
\`\`\`javascript
if (response.authenticationState === 'succeeded') {
  await renderFastlaneMemberExperience(profileData);
} else {
...
- [ ] Verify \`components: ['fastlane']\` in createInstance
- [ ] Confirm \`#watermark-container\` exists in HTML
- [ ] Confirm \`#payment-container\` exists in HTML
- [ ] Log authentication states (succeeded/failed/canceled)
- [ ] Verify PayPal-Request-Id is being sent (Network tab)
- [ ] Check Network tab → Headers → Request Headers

### Backend Debugging
- [ ] Log all incoming request headers
- [ ] Verify PayPal-Request-Id is received (it will be lowercase!)
- [ ] Check CORS allows PayPal-Request-Id header
- [ ] Log PayPal API error responses completely
- [ ] Verify client token generation works
- [ ] Check environment variables are set (CLIENT_ID, SECRET)

### Console Success Messages
You should see these in order:
\`\`\`
Client token obtained
v6 SDK initialized successfully
Fastlane instance created
Watermark rendered
Fastlane initialization complete
Looking up customer: user@example.com
Returning customer found! (or "New customer")
Payment component rendered
Payment token obtained: tokencc_...
Sending PayPal-Request-Id: 1699999999999
...
<html>
<head>
  <title>PayPal Fastlane v6 Checkout</title>
  <meta charset="utf-8">
...
<body>
  <div id="checkout-container">
    <h2>Fastlane Checkout (v6 SDK)</h2>
    ...
    <button id="change-shipping-button" hidden>Change Shipping</button>
    ...
    <!-- Card testing info -->
    <p id="card-testing-info" hidden>
      <small>Test cards: <a href="https://developer.paypal.com/tools/sandbox/card-testing/">PayPal Card Testing</a></small>
    </p>
  </div>
  
  <script>
    let fastlane, sdkInstance, fastlanePaymentComponent, customerProfileData;
    
    // Helper: Get client token from backend
    async function getBrowserSafeClientToken() {
      const response = await fetch('/paypal-api/auth/browser-safe-client-token');
      const data = await response.json();
      return data.accessToken;
    ...
    // Initialize v6 SDK and Fastlane
    async function onPayPalLoaded() {
      try {
        const clientToken = await getBrowserSafeClientToken();
        
        // Create v6 SDK instance
        sdkInstance = await window.paypal.createInstance({
          clientToken,
          pageType: 'checkout',
        ...
        // Create Fastlane from SDK instance
        fastlane = await sdkInstance.createFastlane();
        fastlane.setLocale('en_us');
        
        // Render watermark
        const watermark = await fastlane.FastlaneWatermarkComponent({
          includeAdditionalInfo: true
        });
        ...
        console.error('Fastlane init error:', err);
        alert('Failed to initialize checkout. Please refresh.');
      }
    }
    ...
    async function handleEmailSubmit(e) {
      ...
      try {
        ...
        let shouldRenderMemberExperience = false;
        let profileData;
        ...
        if (customerContextId) {
          ...
          if (response.authenticationState === 'succeeded') {
            shouldRenderMemberExperience = true;
            profileData = response.profileData;
          }
        }
        
        if (shouldRenderMemberExperience) {
          await renderMemberExperience(profileData);
        } else {
          await renderGuestExperience();
        }
    ...
    // Member experience
    async function renderMemberExperience(profileData) {
      customerProfileData = profileData;
      
      if (profileData.shippingAddress) {
        document.getElementById('shipping-info').innerHTML = \`
          <p>\${profileData.shippingAddress.name?.fullName || ''}</p>
          <p>\${profileData.shippingAddress.address?.addressLine1 || ''}</p>
        \`;
        document.getElementById('shipping-display-container').removeAttribute('hidden');
        document.getElementById('change-shipping-button').removeAttribute('hidden');
        
        // Allow changing address
        document.getElementById('change-shipping-button').onclick = async () => {
          const { selectedAddress } = await fastlane.profile.showShippingAddressSelector();
          if (selectedAddress) customerProfileData.shippingAddress = selectedAddress;
        };
      }
    ...
    // Guest experience
    async function renderGuestExperience() {
      document.getElementById('card-testing-info').removeAttribute('hidden');
    ...
    document.getElementById('submit-button').addEventListener('click', async () => {
      try {
        ...
        const orderPayload = {
          ...
          payment_source: {
            card: { single_use_token: token }
          }
        };
        
        if (customerProfileData?.shippingAddress) {
          orderPayload.purchase_units[0].shipping = {
            name: { full_name: customerProfileData.shippingAddress.name?.fullName || 'Guest' },
            address: {
              address_line_1: customerProfileData.shippingAddress.address?.addressLine1,
              admin_area_2: customerProfileData.shippingAddress.address?.adminArea2,
              admin_area_1: customerProfileData.shippingAddress.address?.adminArea1,
        ...
        if (order.status === 'COMPLETED') {
          ...
          const captureRes = await fetch(\`/paypal-api/checkout/orders/\${order.id}/capture\`, {method: 'POST'});
          const capture = await captureRes.json();
          if (capture.status === 'COMPLETED') {
            window.location.href = '/success?order=' + capture.id;
          }
        } else {
...
## Summary of All Changes from Initial Code

### Frontend Changes:
1. Added \`'fastlane'\` to components array
2. Added \`clientMetadataId: crypto.randomUUID()\`
3. Added \`fastlane.setLocale('en_us')\`
4. Added watermark rendering
5. Added \`#watermark-container\` to HTML
6. Added \`#payment-container\` to HTML
7. Fixed authentication flow (render component after auth, not on button click)
8. Fixed component name (\`FastlanePaymentComponent\`)
9. Fixed property name (\`single_use_token\` not \`singleUseToken\`)
10. Added \`PayPal-Request-Id\` header
11. Added comprehensive logging at each step
12. Store profile data and component reference globally
13. Handle all authentication states
14. Get token from already-rendered component

### Backend Changes:
1. Read \`paypal-request-id\` header (lowercase!)
2. Generate fallback UUID if header missing
3. Forward header to PayPal API
4. Added CORS header allowance for PayPal-Request-Id
5. Added comprehensive logging
6. Log full error responses from PayPal API
...
module.exports = { FASTLANE_GUIDE_v4_to_v6 };